...
class SuperJobParser(BaseParser):
    ...
    def parse_vacantions(self, search_params):
        ...
        volume = search_params.get('volume', 1)
        ...
        if response:
            data = response.json()
            return self.detail_data_vacation(data, volume)
        return None

    def detail_data_vacation(self, data, volume=1):
        if not data.get('objects'):
            return []

        for i in range(min(len(data['objects']), volume)):
            vacancy = data['objects'][i]
